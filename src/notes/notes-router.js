...
notesRouter
  ...
  .post(jsonParser, (req, res, next) => {
    const { id, note_name, note_content, folder } = req.body;
    ...
    NotesServices.insertNote(req.app.get("db"), newNote)
      .then((note_name) => {
        res
          .status(201)
          .location(path.posix.join(req.originalUrl, `/${note_name.id}`))
...
notesRouter
  .route("/:note_id")
  .all((req, res, next) => {
    NotesServices.getById(req.app.get("db"), req.params.note_id)
      .then((note) => {
        if (!note) {
